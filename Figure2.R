...
for(r in 1:length(interaction.strength)){
  ...
  carbon.storage[i] <- (carbon.fixation.rate  - respiration.rate - 
                          (nitrogen.fixation.rate*carbon.nitrogen.conversion)) + interaction.rate +
    start.carbon
...
# plot(interaction.strength, net.change.trichome, type = "l", main = "Free trichome vs Colonies", ylab = "Net Change in C Storage", xlab = "Interaction strength (proportion by which C-fixation is affected)")
# lines(interaction.strength, net.change.colony, col = "blue")
...
plot1 <- ggplot(plotdf, aes(x = Ratio, y = Change, color = Group)) +
  ...
  labs(title = "Respiration quotient = 0%",y = expression(paste(Delta,"C"["sto"], "(mol C)")), x = expression(paste("Interaction strength ","(i"["p"],")")))+
...
plot2 <- ggplot(plotdf, aes(x = Ratio, y = Change, color = Group)) +
  geom_line(size = 2, linetype = "solid") +
  geom_hline(yintercept = 0) + 
  theme(panel.background = element_blank(),legend.background = element_blank(), legend.position = c(0.8,0.2), plot.title = element_text(size=16),
        legend.text = element_text(size=12), axis.text = element_text(size=12), axis.title = element_text(size=14),
        panel.border = element_rect(fill = NA, linetype = "solid"), legend.title = element_blank()) + 
  labs(title = "Respiration quotient = 30%",y = expression(paste(Delta,"C"["sto"], "(mol C)")), x = expression(paste("Interaction strength ","(i"["p"],")")))+
  scale_y_continuous(expand = c(0.1, 0.1)) + scale_colour_discrete(labels=c("Colony", "Free trichome"),type = c("darkgreen","grey"))
...
plot3 <- ggplot(plotdf, aes(x = Ratio, y = Change, color = Group)) +
  ...
  theme(panel.background = element_blank(),legend.background = element_blank(), legend.position = c(0.8,0.2), plot.title = element_text(size=16),
        legend.text = element_text(size=12), axis.text = element_text(size=12), axis.title = element_text(size=14),
        panel.border = element_rect(fill = NA, linetype = "solid"), legend.title = element_blank()) + 
  labs(title = "Respiration quotient = 60%",y = expression(paste(Delta,"C"["sto"], "(mol C)")), x = expression(paste("Interaction strength ","(i"["p"],")")))+
  scale_y_continuous(expand = c(0.1, 0.1)) + scale_colour_discrete(labels=c("Colony", "Free trichome"),type = c("darkgreen","grey"))
...
plot4 <- ggplot(plotdf, aes(x = Ratio, y = Change, color = Group)) +
  ...
  theme(panel.background = element_blank(),legend.background = element_blank(), legend.position = c(0.8,0.2), plot.title = element_text(size=16),
        legend.text = element_text(size=12), axis.text = element_text(size=12), axis.title = element_text(size=14),
        panel.border = element_rect(fill = NA, linetype = "solid"), legend.title = element_blank()) + 
  labs(title = "Respiration quotient = 90%",y = expression(paste(Delta,"C"["sto"], "(mol C)")), x = expression(paste("Interaction strength ","(i"["p"],")")))+
  scale_y_continuous(expand = c(0.1, 0.1)) + scale_colour_discrete(labels=c("Colony", "Free trichome"),type = c("darkgreen","grey"))


#####PLOT FIGURE 2#####
plot_grid(plot1,plot2,plot3,plot4, labels = c("(a)","(b)","(c)","(d)"), ncol = 1)